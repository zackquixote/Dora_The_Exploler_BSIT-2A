...
<div class="bmis-content">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        <h1 class="ds-page-title" style="margin:0;"><i class="fas fa-cogs" style="color:var(--c-blue);margin-right:8px;"></i> Barangay Settings</h1>
    </div>
    ...
    <form action="<?= base_url('admin/settings/update') ?>" method="POST">
        ...
        <div style="display:grid;grid-template-columns:1fr;gap:20px;">
                ...
                <div class="ds-card-body">
                    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(250px, 1fr));gap:16px;">
                        <div>
                            ...
                            <input type="text" name="barangay_name" class="ds-input" value="<?= esc($settings['barangay_name'] ?? '') ?>" placeholder="e.g. Tabu">
                        </div>
                        <div>
                            <label class="ds-input-label">Municipality</label>
                            <input type="text" name="municipality" class="ds-input" value="<?= esc($settings['municipality'] ?? '') ?>" placeholder="e.g. Ilog">
                        </div>
                        <div>
                            <label class="ds-input-label">Province</label>
                            <input type="text" name="province" class="ds-input" value="<?= esc($settings['province'] ?? '') ?>" placeholder="e.g. Negros Occidental">
                        </div>
                        <div>
                            <label class="ds-input-label">Contact Number</label>
                            <input type="text" name="contact_number" class="ds-input" value="<?= esc($settings['contact_number'] ?? '') ?>" placeholder="e.g. 0912-345-6789">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="ds-card">
                <div class="ds-card-head" style="display:flex;flex-wrap:wrap;gap:12px;">
                    ...
                    <div style="display:flex;gap:12px;margin-left:auto;">
                        <input type="text" id="resident_search" class="ds-input" style="height:34px;width:200px;" placeholder="Search names...">
                ...
                <div class="ds-card-body">
                    <div style="margin-bottom:12px;font-size:12px;font-weight:700;color:var(--c-navy);text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid var(--border);padding-bottom:8px;">Executive Positions</div>
                    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(250px, 1fr));gap:16px;margin-bottom:24px;">
                        ...
                        ?>
                        <div>
                            <label class="ds-input-label" style="color:var(--c-blue)"><i class="fas fa-user-tie"></i> <?= $label ?></label>
                            <select name="<?= $fieldName ?>" class="ds-select official-select">
                                ...
                                ?>
                                    <option value="<?= $r['id'] ?>"
                                        ...
                                        <?= esc(ucwords($r['last_name'] . ', ' . $r['first_name'])) ?> (<?= esc($r['sitio']) ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div style="margin-bottom:12px;font-size:12px;font-weight:700;color:var(--c-navy);text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid var(--border);padding-bottom:8px;">Sangguniang Barangay Members</div>
                    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(250px, 1fr));gap:16px;">
                        <?php for($i = 1; $i <= 7; $i++):
                            $posName    = "Kagawad $i";
                        ...
                        ?>
                        <div>
                            <label class="ds-input-label"><i class="fas fa-user"></i> <?= $posName ?></label>
            ...
            <div style="text-align:right;">
                <button type="submit" class="ds-btn ds-btn-primary" style="padding:0 32px;"><i class="fas fa-save"></i> Save Settings</button>
            </div>
